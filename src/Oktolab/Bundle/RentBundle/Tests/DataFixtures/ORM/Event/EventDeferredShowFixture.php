 ...
 * @author rt
 */
class EventDeferredShowFixture extends AbstractFixture
{
    /**
     * {@inheritDoc}
     */
    public function load(ObjectManager $manager)
    {
        ...
        $qms = new Qms;
        $qms->setStatus(Qms::STATE_DEFERRED)
            ->setItem($item1)
            ->setCreatedAt(new \DateTime('1991-10-27 11:00:00'))
            ->setUpdatedAt(new \DateTime('1991-10-28 11:00:00'))
            ->setDescription('deferred description');
        $manager->persist($qms);
        $manager->flush();

        $event = new Event();
        $event
            ->setState(Event::STATE_DEFERRED)
